...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    const cat = await prisma.toolCategory.create({ data: { code: body.code, name: body.name, description: body.description || null } });
...
export const PUT: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    if (!isAdmin(payload.role)) return json({ success: false, message: "权限不足" }, { status: 403 });
    const body = await request.json();
    const cat = await prisma.toolCategory.update({ where: { id: body.id }, data: { code: body.code, name: body.name, description: body.description || null } });
    return json({ success: true, category: cat });
