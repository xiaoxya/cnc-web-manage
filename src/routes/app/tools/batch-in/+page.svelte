<script lang="ts">
  ...
  async function submitBatch() {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        success = `入库成功，共处理 ${data.count} 条记录`;
        referenceNo = data.referenceNo || "";
        setTimeout(() => goto("/app/tools"), 1500);
      } else {
        error = data.message || "入库失败";
      }
    } catch { error = "网络错误"; }
    loading = false;
  }
...
<div class="max-w-4xl mx-auto">
  <div class="flex items-center gap-4 mb-6">
    ...
    <h2 class="text-2xl font-bold">批量入库</h2>
  </div>
      ...
      <table class="min-w-full divide-y divide-gray-200 text-sm">
        <thead class="bg-gray-50">
          <tr>
            ...
            <th class="table-header text-right">入库数量</th>
        ...
        <tbody class="divide-y divide-gray-200">
          {#each items as item, i}
            <tr>
              ...
              <td class="table-cell">
                <input class="input" bind:value={item.notes} placeholder="备注" />
              </td>
              <td class="table-cell">
                <button class="text-red-500 hover:text-red-700 text-xs" on:click={() => removeRow(i)}>删除</button>
              </td>
            </tr>
          {/each}
        </tbody>
